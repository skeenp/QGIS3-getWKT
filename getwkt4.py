...
# Load Core
from qgis.core import Qgis, QgsSettings, QgsGeometry, QgsCoordinateReferenceSystem, QgsCoordinateTransform, QgsProject
...
from .getwkt4_dialog import getwkt4Dialog
from .getwkt4_config import getwkt4Config
...
class getwkt4:
    ...
    def __init__(self, iface):
        ...
        locale_path: str = os.path.join(self.plugin_dir , "resources" , "i18n" , f"getwkt4_{self.locale}.qm")
        if os.path.exists(locale_path):
            self.translator = QTranslator()
            self.translator.load(locale_path)
            QCoreApplication.installTranslator(self.translator)
        # Create the dialog (after translation) and keep reference
        self.dlg = getwkt4Dialog()
        self.cfg = getwkt4Config()
        # Declare instance attributes
        self.actions = []
        self.menu = self.tr(u'&Get WKT')
        self.toolbar = self.iface.addToolBar(u'getwkt4')
        self.toolbar.setObjectName(u'getwkt4')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('getwkt4', message)
    ...
    def set_default_button(self):
        if self.s.value("getwkt4/toolmethod") == "EWKT":
            self.toolButton.setDefaultAction(self.btn_ewkt)
        elif self.s.value("getwkt4/toolmethod") == "JSON":
    ...
    def _run(self, out_type):
        ...
        if selected_layer.selectedFeatureCount() == 0:
            ...
            return False
        # Check requested srs
        out_srs_epsg = self.s.value("getwkt4/srid", -1)
        try:
        ...
        multiselect = self.s.value("getwkt4/multiselect", False, bool)
        multiselecttype = self.s.value("getwkt4/multiselecttype")
        ...
        if multiselect:
            ...
            geom = geoms[0]   
        #Setup dp for output
        dp_method = self.s.value("getwkt4/dpmethod")
        if dp_method == "custom":
            try:
                dp_count = int(self.s.value("getwkt4/dpcustom"))
            except ValueError:
                dp_count = None
        elif dp_method == "auto":
            #Determine crs units
            crs_for_dp = out_srs if out_srs else in_srs
            crs_units = crs_for_dp.mapUnits()
            #Allocate auto dp count based on crs units
            dp_count = {
                Qgis.DistanceUnit.Feet: 3,
                Qgis.DistanceUnit.NauticalMiles: 8,
                Qgis.DistanceUnit.Yards: 3,
                Qgis.DistanceUnit.Miles: 8,
                Qgis.DistanceUnit.Millimeters: 0,
                Qgis.DistanceUnit.Centimeters: 2,
                Qgis.DistanceUnit.Meters: 4,
                Qgis.DistanceUnit.Kilometers: 7,
                Qgis.DistanceUnit.Degrees: 8
            }.get(crs_units, None)
        else:
            dp_count = None
        #Collect wkt
        if out_type == 'wkt' or out_type == 'ewkt':
            if multiselect and multiselecttype == 'collection':
