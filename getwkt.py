...
from .getwkt_dialog import getwktDialog
from .getwkt_config import getwktConfig
...
class getwkt:
    ...
    def __init__(self, iface):
        ...
        locale_path: str = os.path.join(self.plugin_dir , "resources" , "i18n" , f"getwkt_{self.locale}.qm")
        if os.path.exists(locale_path):
            self.translator = QTranslator()
            self.translator.load(locale_path)
            QCoreApplication.installTranslator(self.translator)
        # Create the dialog (after translation) and keep reference
        self.dlg = getwktDialog()
        self.cfg = getwktConfig()
        # Declare instance attributes
        self.actions = []
        self.menu = self.tr(u'&Get WKT')
        self.toolbar = self.iface.addToolBar(u'getwkt')
        self.toolbar.setObjectName(u'getwkt')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('getwkt', message)
    ...
    def set_default_button(self):
        if self.s.value("getwkt/toolmethod") == "EWKT":
            self.toolButton.setDefaultAction(self.btn_ewkt)
        elif self.s.value("getwkt/toolmethod") == "JSON":
    ...
    def _run(self, out_type):
        ...
        if selected_layer.selectedFeatureCount() == 0:
            ...
            return False
        # Check requested srs
        out_srs_epsg = self.s.value("getwkt/srid", -1)
        try:
        ...
        multiselect = self.s.value("getwkt/multiselect", False, bool)
        multiselecttype = self.s.value("getwkt/multiselecttype")
        ...
        if multiselect:
            ...
            geom = geoms[0]   
        #Setup dp for output
        dp_method = self.s.value("getwkt/dpmethod")
        if dp_method == "custom":
            try:
                dp_count = int(self.s.value("getwkt/dpcustom"))
            except ValueError:
                dp_count = None
        elif dp_method == "auto":
            #Determine crs units
