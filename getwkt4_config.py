...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'getwkt4_config_base.ui'))

class getwkt4Config(QtWidgets.QDialog, FORM_CLASS):
    """"Setup variables"""
    s = QgsSettings()
    """Get dialog class"""
    def __init__(self, parent=None):
        """Constructor."""
        super(getwkt4Config, self).__init__(parent)
        self.setupUi(self)
        
        # Add integer validator for SRID field
        self.srs_intOutSrs.setValidator(QIntValidator())
        
        # Setup keyboard shortcut
        self.btnAccept.setShortcut(QKeySequence("Escape"))
        
        self.btnAccept.clicked.connect(self.handleHide)
        #Load saved settings
        dpmethod = self.s.value("getwkt4/dpmethod")
        if dpmethod == 'auto':
        ...
        #Custom value
        val = self.s.value("getwkt4/dpcustom")
        if not val:
            val = 10
        ...
        tool = self.s.value("getwkt4/toolmethod")
        ...
        if not multiselecttype:
            multiselecttype = 'multi'
            self.s.setValue("getwkt4/multiselecttype", multiselecttype)
        ...
        if multiselect:
            ...
            self.ms_radGeomCollection.setEnabled(True)
        else:
            self.ms_radMulti.setEnabled(False)
            self.ms_radGeomCollection.setEnabled(False)
        #SRID value
        val = self.s.value("getwkt4/srid")
        if not val:
            val = -1
        ...
        self.ms_chkMultiExport.checkStateChanged.connect(self.ms_handleMultiSelectChange)
    ...
    def setupUi(self, Dialog):
        ...
        super(getwkt4Config, self).setupUi(Dialog)
    ...
    def tool_handleDefaultChange(self, value):
        ...
        #Update dp setting
        self.s.setValue("getwkt4/toolmethod", self.tool_cmbDefault.currentText())
    
    def dp_handleChange(self, t):
        """Handle change to dp method"""
        #Update dp setting
        self.s.setValue("getwkt4/dpmethod", t)
    ...
    def dp_handleChange_CustomValue(self):
        ...
        self.s.setValue("getwkt4/dpcustom",value)
        
    def ms_handleMultiSelectChange(self, state):
        """Handle change to multi-selection export option"""
        ms_enabled = state == Qt.CheckState.Checked
        # Update multi export setting
        self.s.setValue("getwkt4/multiselect", ms_enabled)
    ...
    def ms_handleChange(self, t):
        """Handle change to dp method"""
        #Update dp setting
        self.s.setValue("getwkt4/multiselecttype", t)
    ...
    def srs_handleChange_SRID(self):
        ...
        try:
            ...
            value = -1
        self.s.setValue("getwkt4/srid",value)